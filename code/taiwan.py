...
def fun_1():    #定義爬蟲用的function
    ...
    file_path = '備份/'+name+'_data_' + weekday + '.xlsx'  #備份路徑與黨名
    ...
    for row in range (url_dict.shape[0]): 
        ...
        ex_format = url_dict.iloc[row,3] #撥回的格式
        ...
        while price_table.shape[0] <= 5 or price_table.shape[1] <= 1 : #用while迴圈嘗試讀取網頁，如果rows<=5或是columns<=1則重來
            try:            
                ...
                price_table.set_index('日期',inplace=True)
            ...
            if ex_url != '':
                ...
                try:
                    ...
                    #表格的日期原為民國，以for迴圈更改為西元
                    for ex_date in range(len(ex_Dividend)):
                        ex_Dividend.iloc[ex_date,0]=ex_Dividend.iloc[ex_date,0].replace(ex_Dividend.iloc[ex_date,0][0:3], str(int(ex_Dividend.iloc[ex_date,0][0:3]) + 1911))
                    
                    ex_Dividend.set_index('日期',inplace=True) #以日期為索引
